 ...
 *Gaussian kernel.
 **/
inline std::vector<std::vector<double> > create_Gaussian_filter(size_t pixel_radius, double sigma) {
  bool dbg = false;
...
/**
 * \class Persistence_image Persistence_image.h gudhi/Persistence_image.h
...
class Persistence_image {
 public:
   ...
   * (1) A vector of tuple of doubles (representing persistence intervals and multiplicities). All other parameters are optional. They are:
   ...
   *std::numeric_limits<double>::max(), in which case the program compute the values based on the data.
   **/
  Persistence_image(const std::vector<std::tuple<double, double, double> >& interval,
   ...
   *std::numeric_limits<double>::max(), in which case the program compute the values based on the data.
   **/
  Persistence_image(const std::vector<std::pair<double, double> >& interval,
                        std::vector<std::vector<double> > filter = create_Gaussian_filter(5, 1),
                        bool erase_below_diagonal = false, size_t number_of_pixels = 1000,
                        double min_ = std::numeric_limits<double>::max(),
                        double max_ = std::numeric_limits<double>::max());
   ...
   *std::numeric_limits<double>::max(), in which case the program compute the values based on the data.
   **/
  Persistence_image(const char* filename, std::vector<std::vector<double> > filter = create_Gaussian_filter(5, 1),
                        bool erase_below_diagonal = false, size_t number_of_pixels = 1000,
                        double min_ = std::numeric_limits<double>::max(),
                        double max_ = std::numeric_limits<double>::max(),
   ...
   **/
  void compute_mean(const std::vector<Persistence_image*>& maps);
   ...
   * If this is not the case, the program will throw an exception.
   **/
  void compute_median(const std::vector<Persistence_image*>& maps);

  /**
   * Compute a percentage of active (i.e) values above the cutoff of a collection of heat maps.
   **/
  void compute_percentage_of_active(const std::vector<Persistence_image*>& maps, size_t cutoff = 1);
   ...
   **/
  inline bool check_if_the_same(const Persistence_image& second) const {
    bool dbg = false;
   ...
   **/
  bool operator==(const Persistence_image& rhs) const {
    bool dbg = false;
   ...
   **/
  bool operator!=(const Persistence_image& rhs) const { return !((*this) == rhs); }

  template <typename Operation_type>
  friend Persistence_image operation_on_pair_of_heat_maps(const Persistence_image& first,
                                                              const Persistence_image& second,
  ...
  /**
   * Multiplication of Persistence_image by scalar (so that all values of the heat map gets multiplied by that
   *scalar).
   **/
  Persistence_image multiply_by_scalar(double scalar) const {
    Persistence_image result;
  ...
  /**
   * This function computes a sum of two objects of a type Persistence_image.
   **/
  friend Persistence_image operator+(const Persistence_image& first, const Persistence_image& second) {
    return operation_on_pair_of_heat_maps(first, second, std::plus<double>());
  }
  /**
   * This function computes a difference of two objects of a type Persistence_image.
   **/
  friend Persistence_image operator-(const Persistence_image& first, const Persistence_image& second) {
    return operation_on_pair_of_heat_maps(first, second, std::minus<double>());
  }
  /**
   * This function computes a product of an object of a type Persistence_image with real number.
   **/
  friend Persistence_image operator*(double scalar, const Persistence_image& A) {
    return A.multiply_by_scalar(scalar);
  }
  /**
   * This function computes a product of an object of a type Persistence_image with real number.
   **/
  friend Persistence_image operator*(const Persistence_image& A, double scalar) {
    return A.multiply_by_scalar(scalar);
  }
  /**
   * This function computes a product of an object of a type Persistence_image with real number.
   **/
  Persistence_image operator*(double scalar) { return this->multiply_by_scalar(scalar); }
  /**
   * += operator for Persistence_image.
   **/
  Persistence_image operator+=(const Persistence_image& rhs) {
    *this = *this + rhs;
    return *this;
  }
  /**
   ...
   * *= operator for Persistence_image.
   **/
  Persistence_image operator*=(double x) {
    *this = *this * x;
    return *this;
  }
  /**
   ...
   * The parameter of this function is a const reference to an object of a class Persistence_image.
   ...
   **/
  double distance(const Persistence_image& second_, double power = 1) const;
   ...
   **/
  void compute_average(const std::vector<Persistence_image*>& to_average);
   ...
   **/
  double compute_scalar_product(const Persistence_image& second_) const;
 ...
 protected:
  // private methods
  std::vector<std::vector<double> > check_and_initialize_maps(const std::vector<Persistence_image*>& maps);
  ...
  void construct(const std::vector<std::tuple<double, double, double> >& intervals_,
                 std::vector<std::vector<double> > filter = create_Gaussian_filter(5, 1),
                 bool erase_below_diagonal = false, size_t number_of_pixels = 1000,
                 double min_ = std::numeric_limits<double>::max(), double max_ = std::numeric_limits<double>::max());
...
Persistence_image<Scalling_of_kernels>::Persistence_image(
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::construct(const std::vector<std::pair<double, double> >& intervals_,
...
// if min_ == max_, then the program is requested to set up the values itself based on persistence intervals
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::construct(const std::vector<std::tuple<double, double, double> >& intervals_,
                                                           std::vector<std::vector<double> > filter,
                                                           bool erase_below_diagonal, size_t number_of_pixels,
                                                           double min_, double max_) {
  bool dbg = false;
  ...
  if (min_ == max_) {
    ...
    for (size_t i = 0; i != intervals_.size(); ++i) {
      if (std::get<0>(intervals_[i]) < min_) min_ = std::get<0>(intervals_[i]);
      if (std::get<1>(intervals_[i]) > max_) max_ = std::get<1>(intervals_[i]);
    }
    // now we have the structure filled in, and moreover we know min_ and max_ values of the interval, so we know the
    // range.
  ...
  for (size_t pt_nr = 0; pt_nr != intervals_.size(); ++pt_nr) {
    // compute the value of intervals_[pt_nr] in the grid:
    int x_grid =
        static_cast<int>((std::get<0>(intervals_[pt_nr]) - this->min_) / (this->max_ - this->min_) * number_of_pixels);
    int y_grid =
        static_cast<int>((std::get<1>(intervals_[pt_nr]) - this->min_) / (this->max_ - this->min_) * number_of_pixels);

    if (dbg) {
      std::clog << "point : " << std::get<0>(intervals_[pt_nr]) << " , " << std::get<1>(intervals_[pt_nr]) << std::endl;
      std::clog << "x_grid : " << x_grid << std::endl;
      std::clog << "y_grid : " << y_grid << std::endl;
    }
    ...
    double scaling_value = this->f({std::get<0>(intervals_[pt_nr]), std::get<1>(intervals_[pt_nr])});
    ...
    for (size_t i = 0; i != filter.size(); ++i) {
      for (size_t j = 0; j != filter.size(); ++j) {
        ...
        if (((x_grid + i) >= 0) && (x_grid + i < this->heat_map.size()) && ((y_grid + j) >= 0) &&
          ...
          this->heat_map[y_grid + j][x_grid + i] += scaling_value * filter[i][j] * std::get<2>(intervals_[pt_nr]);
          if (dbg) {
            std::clog << "Position : (" << x_grid + i << "," << y_grid + j
                      << ") got increased by the value : " << filter[i][j] << std::endl;
          }
...
template <typename Scalling_of_kernels>
Persistence_image<Scalling_of_kernels>::Persistence_image(
    const std::vector<std::tuple<double, double, double> >& interval, std::vector<std::vector<double> > filter,
    bool erase_below_diagonal, size_t number_of_pixels, double min_, double max_) {
  this->construct(interval, filter, erase_below_diagonal, number_of_pixels, min_, max_);
  this->set_up_parameters_for_basic_classes();
}
...
// template <typename Scalling_of_kernels>
// Persistence_image<Scalling_of_kernels>::Persistence_image(const char* filename,
//                                                                   std::vector<std::vector<double> > filter,
//                                                                   bool erase_below_diagonal, size_t number_of_pixels,
...
template <typename Scalling_of_kernels>
std::vector<std::vector<double> > Persistence_image<Scalling_of_kernels>::check_and_initialize_maps(
    const std::vector<Persistence_image*>& maps) {
  // checking if all the heat maps are of the same size:
  for (size_t i = 0; i != maps.size(); ++i) {
    if (maps[i]->heat_map.size() != maps[0]->heat_map.size()) {
      std::cerr << "Sizes of Persistence_image are not compatible. The program will terminate now \n";
      throw "Sizes of Persistence_image are not compatible. The program will terminate now \n";
    }
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::compute_median(const std::vector<Persistence_image*>& maps) {
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::compute_mean(const std::vector<Persistence_image*>& maps) {
  std::vector<std::vector<double> > heat_maps = this->check_and_initialize_maps(maps);
  for (size_t i = 0; i != heat_maps.size(); ++i) {
    for (size_t j = 0; j != heat_maps[i].size(); ++j) {
      double mean = 0;
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::compute_percentage_of_active(
    const std::vector<Persistence_image*>& maps, size_t cutoff) {
  std::vector<std::vector<double> > heat_maps = this->check_and_initialize_maps(maps);
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::print_to_file(const char* filename) const {
  std::ofstream out;
  out.open(filename);
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::load_from_file(const char* filename) {
  bool dbg = false;
...
template <typename Scalling_of_kernels>
std::vector<double> Persistence_image<Scalling_of_kernels>::vectorize(int number_of_function) const {
...
template <typename Scalling_of_kernels>
double Persistence_image<Scalling_of_kernels>::distance(const Persistence_image& second, double power) const {
  if (this->discrete) {
...
template <typename Scalling_of_kernels>
double Persistence_image<Scalling_of_kernels>::project_to_R(int number_of_function) const {
  double result = 0;
  for (size_t i = 0; i != this->heat_map.size(); ++i) {
    for (size_t j = 0; j != this->heat_map[i].size(); ++j) {
...
template <typename Scalling_of_kernels>
void Persistence_image<Scalling_of_kernels>::compute_average(
    const std::vector<Persistence_image*>& to_average) {
  this->compute_mean(to_average);
}

template <typename Scalling_of_kernels>
double Persistence_image<Scalling_of_kernels>::compute_scalar_product(const Persistence_image& second) const {
  if (discrete) {
    // first we need to check if (*this) and second are defined on the same domain and have the same dimensions:
    if (!this->check_if_the_same(second)) {
      std::cerr << "The persistence images are of non compatible sizes. We cannot therefore compute distance between "
